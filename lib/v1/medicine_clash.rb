...
module MedicineClashKata
  ...
  class Medicine
    ...
    def dates_prescribed(day_count)
      prescriptions_in_range(day_count).flat_map do |p|
  ...
  class Patient
    ...
    def clash(medicine_names, days_back)
      medicines_taken = medicines.select { |m| medicine_names.include?(m.name) }
      return [] if medicines_taken.empty?

      all_dates = medicines_taken.map { |m| m.dates_prescribed_in_effective_range(days_back) }
