...
class SupermarketTest < Minitest::Test
  def test_discounts
    ...
    apples = Product.new("apples", :kilo, 1.99, discount: { type: :percent, percent: 20 })
    receipt.add_receipt_item(apples, 2.5)

    rice = Product.new("rice", :each, 2.49, discount: { type: :percent, percent: 10 })
  ...
  def test_total_is_sum_of_line_items
    ...
    toothbrush = Product.new("toothbrush", :each, 0.33, discount: { type: :percent, percent: 20 })
    receipt.add_receipt_item(toothbrush, 1)

    toothpaste = Product.new("toothpaste", :each, 0.33, discount: { type: :percent, percent: 20 })
