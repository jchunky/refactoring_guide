...
character = DnDchars.new(nil,nil,nil,nil,nil,nil,nil,nil,
                         nil,nil, nil,nil,nil,nil,nil,nil, nil,
                         nil,nil, nil,nil
)

# choosing a race
character.race = racepick

puts "Your character is going to be a #{character.race}.\n\n"

# choosing a class
character.class_of_char = classpick
puts "Your character is going to be a #{character.class_of_char}.\n\n"

# choosing a background
character.background = backgroundpick
puts "Your character's background is #{character.background}.\n\n"

# gathers input on character name and level
puts "What is your character's name?"
character.charname = get_input("Conan")

puts "What level is your character? "
character.level = get_input("1").to_i

# calculating proficiency based on level
character.prof = proficiency(character.level)
puts "Your character's proficiency is #{character.prof}.\n\n"

# rolling for stats
stat_array = []
stat_array = statroll

# choosing which rolls to apply to each stat
chosen_stats = []
chosen_stats = statpick(stat_array)
character.str = chosen_stats[0]
character.dex = chosen_stats[1]
character.con = chosen_stats[2]
character.int = chosen_stats[3]
character.wis = chosen_stats[4]
character.cha = chosen_stats[5]

# stat modifier calculations
character.strmod = my_modcalc(character.str)
character.dexmod = my_modcalc(character.dex)
character.conmod = my_modcalc(character.con)
character.intmod = my_modcalc(character.int)
character.wismod = my_modcalc(character.wis)
character.chamod = my_modcalc(character.cha)

puts "Your character's Strength modifier is #{character.strmod}, Dexterity modifier is #{character.dexmod},
Constitution modifier is #{character.conmod}, Intelligence modifier is #{character.intmod}, Wisdom modifier
is #{character.wismod}, and Charisma modifier is #{character.chamod}.\n\n"

#hitpoint calculation
character.hitpoints = hitpointcalculator(character.class_of_char, character.level, character.conmod)
puts "Your character's maximum hitpoints are #{character.hitpoints}.\n\n"

# populating skills into the character.skills hash
character.skills = skillpopulator

character.skills.each do |skill, value|
  puts "Your character's #{skill} modifier is #{value}\n"
end

prof_array =[]
prof_array = proficiency_by_class(character.class_of_char)

prof_array.each do |prof|
  puts prof
end
